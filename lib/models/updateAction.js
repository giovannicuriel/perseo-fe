...
function buildUpdateOptions(action, event) {
    ...
    for (var i = 0; i < action.parameters.attributes.length; i++) {
        ...
        if (action.parameters.attributes[i].type !== undefined) {
            ...
            // Retrieve variable name. This regexp should match only '${var}'.
            // Anything else should be considered a string.
            var tokens = /^\$\{(.*?)\}$/g.exec(action.parameters.attributes[i].value);
            if (tokens !== null) {
              if ((tokens[1] + '__type') in event) {
                attr.type = event[tokens[1] + '__type'];
              }
            }
        }
    ...
    options.attributes.push(timeInstantMeta);
